...
#define FF_DRV_VERSION              "V3.2.0-20230225"
...
static int ff_ctl_init_driver(ff_context_t *ff_ctx)
{
    ...
    if (ff_ctx->b_screen_onoff_event) {
        ...
        if (ret) {
            FF_LOGE("init notifier callback fail");
        }
    }
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FF_IOC_GET_SPICLK_HZ:
        FF_LOGI("get spi-clk hz:%d", ff_ctx->spi_clk_hz);
...
static int fts_notifier_callback_init(ff_context_t *ff_ctx)
{
    ...
    if (ret < 0) {
        FF_LOGE("[DRM]mtk_disp_notifier_register fail: %d\n", ret);
    }
#elif IS_ENABLED(CONFIG_FB)
    ...
    if (ret) {
        FF_LOGE("[FB]Unable to register fb_notifier: %d", ret);
    }
#endif
    FF_LOGV("'%s' exit.", __func__);
    return ret;
...
{
    ff_context_t *ff_ctx = client_data;
    ...
    FF_LOGD("Notification type:%d, early_trigger:%d",
    ...
    case DRM_PANEL_EVENT_BLANK_LP:
        FF_LOGD("received lp event\n");
        break;

    case DRM_PANEL_EVENT_FPS_CHANGE:
        FF_LOGD("Received fps change old fps:%d new fps:%d\n",
            notification->notif_data.old_fps,
            notification->notif_data.new_fps);
        break;

    default:
        FF_LOGD("notification serviced :%d\n",
            notification->notif_type);
        break;
    }
}
...
#else
static void panel_notifier_callback(enum panel_event_notifier_tag tag,
                 struct panel_event_notification *notification, void *client_data)
{
        ff_context_t *ff_ctx = client_data;
...
static int fts_notifier_callback_init(ff_context_t *ff_ctx)
{
    ...
    int ret = 0;
    FF_LOGV("'%s' enter.", __func__);
#if IS_ENABLED(CONFIG_DRM) && IS_ENABLED(CONFIG_DRM_PANEL)
    ret = drm_check_dt(ff_ctx);
    if (ret) FF_LOGE("parse drm-panel fail");
    if (active_panel) {
        cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
            PANEL_EVENT_NOTIFIER_CLIENT_PRIMARY_TOUCH, active_panel,
            &panel_notifier_callback, ff_ctx);
        if (!cookie) FF_LOGE("[DRM]panel_event_notifier_register fail\n", ret);

        FF_LOGD("registered for panel notifications panel: 0x%x\n", active_panel);
	}
#endif
	ff_ctx->notifier_cookie = cookie;
    FF_LOGV("'%s' exit.", __func__);
    return ret;
}
...
static int fts_notifier_callback_exit(ff_context_t *ff_ctx)
{
	...
	if (ff_ctx->notifier_cookie)
		panel_event_notifier_unregister(ff_ctx->notifier_cookie);
